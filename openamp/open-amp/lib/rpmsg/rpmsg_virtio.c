		    ...
		    struct rpmsg_virtio_shm_pool *shpool)
{
    ...
    PRINTF("rpmsg_init_vdev: enter rpmsg_virtio_wait_remote_ready\n\r");
    rpmsg_virtio_wait_remote_ready(rvdev);
    PRINTF("rpmsg_init_vdev: leave rpmsg_virtio_wait_remote_ready\n\r");

	/* Create virtqueues for remote device */
    PRINTF("rpmsg_init_vdev: enter rpmsg_virtio_create_virtqueues\n\r");
	status = rpmsg_virtio_create_virtqueues(rvdev, 0, RPMSG_NUM_VRINGS,
						vq_names, callback);

	if (status != RPMSG_SUCCESS) {
        PRINTF("rpmsg_init_vdev: rpmsg_virtio_create_virtqueues failed with %d\n\r", status);
		return status;
    }

    PRINTF("rpmsg_init_vdev: rpmsg_virtio_create_virtqueues success with %d\n\r", status);
    ...
    PRINTF("rpmsg_init_vdev: dev_features=0x%x\n\r", dev_features);
	...
	if ((dev_features & (1 << VIRTIO_RPMSG_F_NS))) {
        PRINTF("rpmsg_init_vdev: creating namespace endpoint add=0x%x\n\r", RPMSG_NS_EPT_ADDR);
		rpmsg_init_ept(&rdev->ns_ept,
                       "NS",
                        RPMSG_NS_EPT_ADDR,
                        RPMSG_NS_EPT_ADDR,
                        rpmsg_virtio_ns_callback,
